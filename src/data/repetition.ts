...
export const repetitionByTopic: Record<string, Question[]> = {
  ...
  'lineare-algebra-2': [
    makeMcq('rep-la-1', 'Was ist eine quadratische Matrix?',
      'Eine Matrix mit gleich vielen Zeilen und Spalten.',
      ['Eine Matrix mit 1 Zeile', 'Eine Matrix mit negativen Einträgen', 'Eine Matrix mit Determinante 0'],
      'Bei einer quadratischen Matrix gilt: Zeilenanzahl = Spaltenanzahl (z.B. 2×2, 3×3).',
      1, ['matrix', 'definition']),
    makeMcq('rep-la-2', 'Berechne det(((1, 3), (5, 7))).',
      '−8', ['8', '22', '−22'],
      'det = 1·7 − 3·5 = 7 − 15 = −8.',
      2, ['determinante']),
    makeMcq('rep-la-3', 'Was bedeutet es, wenn Eigenwerte komplex sind?',
      'Die Matrix hat keine „bevorzugten" reellen Richtungen — sie rotiert.',
      ['Die Matrix hat keine Lösung', 'Die Matrix ist symmetrisch', 'Die Matrix ist die Nullmatrix'],
      'Bei Drehungen oder Rotationen sind Eigenwerte imaginär — die Matrix transformiert Vektoren nicht durch Skalierung, sondern durch Rotation.',
      3, ['eigenwert', 'komplex']),
    makeMcq('rep-la-4', 'Welche Aussage über symmetrische Matrizen ist richtig?',
      'Sie haben immer reelle Eigenwerte.',
      ['Sie sind immer invertierbar', 'Sie haben Determinante 1', 'Sie sind Dreiecksmatrizen'],
      'Der Spektralsatz garantiert: symmetrische Matrizen (A = Aᵀ) haben reelle Eigenwerte und orthogonale Eigenvektoren.',
      3, ['symmetrisch', 'eigenwert']),
  ],
  integral: [
    makeMcq('rep-int-1', 'Was ist die Stammfunktion von f(x) = 3x²?',
      'F(x) = x³ + C',
      ['F(x) = 3x + C', 'F(x) = x² + C', 'F(x) = 6x + C'],
      'Potenzregel rückwärts: x² → x³/1 = x³. Aber achte auf den konstanten Faktor 3: F(x) = 3·(x³/3) = x³.',
      2, ['integral', 'stammfunktion']),
    makeMcq('rep-int-2', 'Berechne ∫₋₂² x³ dx.',
      '0',
      ['32/4', '16', '−16'],
      'x³ ist ungerade, symmetrisches Intervall [−2, 2]. Positiv und negativ heben sich auf → 0.',
      2, ['integral', 'symmetrie']),
    makeMcq('rep-int-3', 'Was ist ∫ (sin(x) + cos(x)) dx?',
      '−cos(x) + sin(x) + C',
      ['cos(x) − sin(x) + C', '−sin(x) + cos(x) + C', 'sin(x) − cos(x) + C'],
      'Summenregel: ∫ sin dx + ∫ cos dx = −cos(x) + sin(x) + C.',
      3, ['integral', 'grundintegrale']),
    makeMcq('rep-int-4', 'Wie groß ist das Volumen eines Kegels mit Höhe h und Radius R (Rotation von f(x) = R·x/h zwischen 0 und h)?',
      '(1/3) · π · R² · h',
      ['π · R² · h', '(2/3) · π · R² · h', 'π · R · h'],
      'V = π ∫₀ʰ (R·x/h)² dx = π · (R²/h²) · [x³/3]₀ʰ = π · R² · h / 3 = (πR²h)/3. Die klassische Kegel-Volumenformel.',
      3, ['integral', 'rotation']),
  ],
  // Stil & Ausdruck
